...
function addListernerToAttachCellProperties(cell) {
    cell.addEventListener("click", (e) =>{
        ...
        // Apply properties to UI properties container
...
function getCellAndCellProp(address) {
    ...
    let cell = document.querySelector(`.cell[rid="${rid}"][cid="${cid}"]`); // for UI
    let cellProp = sheetDB[rid][cid]; // for Storage object
    return [cell, cellProp];
}

function decodeRIDCIDFromAddress(address) {
    // if address = "A1"
    let rid = Number(address.slice(1)-1); // we get "1" in the form of string and we get 0 in the form number.
    let cid = Number(address.charCodeAt(0))-65; // "A" -> 65 -> 0(final o/p).
    return [rid,cid];
}
